...
var AlexaAPI = function(accessKey, secretKey) {
    var self = this;
...
AlexaAPI.prototype.get = function(request, cb) {
    ...
    var self = this;
    var request_options = { qs: request };
    if (process.env.http_proxy) request_options.proxy = process.env.http_proxy;

    return URLrequest.get(util.format('http://%s', self.apiDomain),
        request_options, cb);
};
